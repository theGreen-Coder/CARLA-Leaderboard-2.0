...
Due to the length of the routes as well as large number of scenarios per route the scores on this benchmark are much lower than on Bench2Drive and other benchmarks.
...
The benchark can be found [here](leaderboard/data/longest6.xml) and the individual route files [here](leaderboard/data/longest6_split). Unlike the leaderboard 1.0 version there are no modifications to the CARLA leaderboard code. Longest6 is a training benchmark so training on Town 01-06 is allowed.
...
--id test_tf_000_0
```
and fill in the parameters train.py afterward.
